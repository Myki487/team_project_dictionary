...
def avg_one_student_one_subject(journal: dict):
    """Обчислює середній бал конкретного студента за конкретною дисципліною."""
    if not journal:
        print("\nЖурнал порожній.")
        return
    ...
    perf = journal[student_id].get("performance", {})
    grades = perf.get(subject)

    if not grades:
        print(f"Немає оцінок з дисципліни '{subject}' для цього студента.")
        return

    valid_grades = [g for g in grades if isinstance(g, (int, float))]
    if not valid_grades:
        print("Помилка: у дисципліні є некоректні оцінки (не числа).")
        return

    avg = float(sum(valid_grades)) / len(valid_grades)
    s = journal[student_id]
    print(f"\nAVG студента {s['last_name']} {s['first_name']} ({subject}) = {avg:.2f}")
